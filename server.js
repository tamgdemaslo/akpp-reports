...
app.get('/vin-search', async (req, res) => {
    ...
    const monthOfProduction = r0.modelMonth || r0.startMonth || r0.finishMonth || '';
    const gear = r0.gear || {};
    ...
    const promptText = [
        ...
        `Год: ${r0.modelYear || ''}`,
        `VIN: ${vin_data.vin || ''}`,
        `Месяц выпуска: ${monthOfProduction ? String(monthOfProduction) : ''}`,
    ...
    const systemPrompt =
        ...
        "Не ограничивайся только семейством (например, «8HP50») — предпочитай точный OEM-код; если точный OEM-код нельзя вывести даже с VIN/месяцем, перечисли возможные OEM-коды и напиши, каких данных не хватает (код КПП из ETK/наклейки/part number).\n" +
